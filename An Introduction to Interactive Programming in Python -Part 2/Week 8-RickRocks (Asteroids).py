...
soundtrack = simplegui.load_sound("https://www.dropbox.com/s/b2wfy8ayrqdksl0/Two%20Steps%20From%20Hell%20%5Bmqms2%5D.mp3?dl=1")
...
def group_collide(group, other_object):
    ...
    for b in set(group):
        if b.collide(other_object):
            group.remove(b)
            explosion_group.add(Sprite(b.pos, [0, 0], 0, 0, explosion_image, explosion_info))
...
def group_group_collide(group, other_group):
    for c in set(other_group):
        if group_collide(group, c):
            other_group.remove(c)
            return True
    return False
...
def click(pos):
    ...
    #    soundtrack.pause()
def draw(canvas):
    global time, started, lives, rock_group, score, exposion_group
...
def rock_spawner():
    ...
    if score > 10:
        ...
        rock_vel = [random.random() * .6 + 2.3, random.random() * .6 + 2.3]
    else:
        rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
    ...
    if started:
        if dist(my_ship.get_position(), rock_pos) > asteroid_info.get_radius() + my_ship.get_radius():
