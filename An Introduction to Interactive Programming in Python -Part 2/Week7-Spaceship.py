...
class Ship:
    ...
    def update(self):
        ...
        self.vel[1] *= 0.99
        if self.thrust:
            self.vel[0] += angle_to_vector(self.angle)[0] * 0.2
            self.vel[1] += angle_to_vector(self.angle)[1] * 0.2
    ...
    def shoot(self):
        ...
        missile_vel[0] = self.vel[0] + angle_to_vector(self.angle)[0] * 5
        missile_vel[1] = self.vel[1] + angle_to_vector(self.angle)[1] * 5
...
def keydown(key):
    if simplegui.KEY_MAP["up"] == key:
        ...
        #my_ship_thrust.canvas.draw_image(ship_image, self.image_center, self.image_size, self.pos, self.image_size, self.angle)
    elif simplegui.KEY_MAP["left"] == key:
        my_ship.angle_vel -= angular_velocity_INC
...
def draw(canvas):
    ...
    canvas.draw_text("Lives: " + str(lives), [0, 600], 22, "white")
    canvas.draw_text("Score: " + str(score), [720, 600], 22, "white")
# timer handler that spawns a rock    
def rock_spawner():
    global a_rock
    vel = [random.randrange(-15, 15) / 10.0, random.randrange(-15, 15) / 10.0]
...
#my_ship_thrust = Ship([WIDTH / 2, HEIGHT / 2], [0, 0], 0, ship_image, ship_info_thrust)
