...
class Apocalypse(poc_grid.Grid):
    ...
    def move_humans(self, zombie_distance_field):
        ...
        for human in self._human_list:
            ...
            new_positions = (human[0], human[1])
            for idx in range(len(neighbors)):
                if dist < zombie_distance_field[neighbors[idx][0]][neighbors[idx][1]] and \
                    self.is_empty(neighbors[idx][0], neighbors[idx][1]):
                    dist = zombie_distance_field[neighbors[idx][0]][neighbors[idx][1]]
                    new_positions = (neighbors[idx][0], neighbors[idx][1])
            new_human_list.append(new_positions)
    ...
    def move_zombies(self, human_distance_field):
        ...
        for zombie in self._zombie_list:
            ...
            new_positions = (zombie[0], zombie[1])
            for idx in range(len(neighbors)):
                if dist > human_distance_field[neighbors[idx][0]][neighbors[idx][1]] and \
                    self.is_empty(neighbors[idx][0], neighbors[idx][1]):
                    dist = human_distance_field[neighbors[idx][0]][neighbors[idx][1]]
                    new_positions = (neighbors[idx][0], neighbors[idx][1])               
            new_zombie_list.append(new_positions)
